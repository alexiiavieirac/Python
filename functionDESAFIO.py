...
# Data Science e Intligência Artificial usa muito isso.

# 1. Quando criamos um modelo de previsão, precisamos treinar esse modelo e testar para ver se é um bom modelo ou não;
# 2. Temos então que pegar os nossos dados e dividir em dois pedaços, uma lista de treino e uma de teste;
# 3. Vamos então pensar no exemplo de um modelo que tenta identificar qual o valor justo de um imóvel de acord com o tamanho do imóvel. Temos então duas listas:
#   Lista 1: preços reais dos imóveis;
#   Lista 2: tamanho do imóvel.
# 4. Vamos criar então uma função que recebe duas listas como entrada e divide cada uma dessas listas em dois, um pedaço de treino e um de teste. O percentual
# que a lista vai ser dividida é definida por um fator (que será parâmetro da function).

precos_imoveis = [2.17, 1.54, 1.45, 1.94, 2.37, 2.3, 1.79, 1.8, 2.25, 1.37]
tamanho_imoevis = [207, 148, 130, 203, 257, 228, 160, 194, 232, 147]
...
def separar_listas(precos, tamanho, fator=0.1):
    if len(precos) == len(tamanho):
        ...
        print('As listas de preços e tamanhos não pussem o mesmo tamanho.')
        return
